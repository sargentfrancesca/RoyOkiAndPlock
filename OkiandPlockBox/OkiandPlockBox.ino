  ...
  void Update() {
    ...
    if((millis() - lUpdate) > uInterval)  // time to update
    {
     ...
     bool right_state = true;
      ...
      if (left_state && l_average > 350) {
        int new_val = map(l_average, 350, 500, mouthmin - 10, mouthmax);        
        pwm.setPWM(okiMouth, 0, l_average);
        pwm.setPWM(plockLeftTache, 0, tachenatural); 
        pwm.setPWM(plockRightTache, 0, tachemin);
      } else if (right_state && r_average > 400) {
        int smaller = map(r_average, 400, 700, tachemin, tachemax);
